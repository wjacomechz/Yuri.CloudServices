...
{
    public static class RecaptchaGoogle
    {
        public static bool Validar(ref JCHNETReCaptchaGoogleDto post, ref string mensaje)
        {
            ...
            {
                post.EncodedResponse = Conversions.NothingToString(post.EncodedResponse);
                if ((Conversions.NothingToString(post.ValorReCaptcha) != post.EncodedResponse.Length.ToString()) || post.EncodedResponse.Length == 0)
                {
        ...
        private static bool ReCaptchaGoogleReply(JCHNETReCaptchaGoogleDto post, ref string mensaje)
        {
            try
            {
                ...
                if (post.ReCaptchaClaveSitioWeb == null | post.ReCaptchaClaveComGoogle == null)
                    return true;
                if (Conversions.NothingToString(post.EncodedResponse) == "")
                {
